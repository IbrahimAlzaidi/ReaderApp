    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        Column(verticalArrangement = Arrangement.Center,
            ...
            if (showLoginForm.value) UserForm() { email, password ->
                viewModel.signUserWithEmailAndPassword(email, password) {
            ...
            Row(modifier = Modifier.padding(15.dp),
                ...
                val text2 = if (showLoginForm.value) "New User?" else "Already a User?"
                Text(text2)
...
fun UserForm(
    ...
    Column(modifier,
        horizontalAlignment = Alignment.CenterHorizontally) {
        if (isCreateAccount) Text(stringResource(id = R.string.create_acct),
            modifier = Modifier
                .padding(start = 10.dp, end = 10.dp, bottom = 10.dp, top = 4.dp)
                .align(Alignment.Start)) else Text("")
